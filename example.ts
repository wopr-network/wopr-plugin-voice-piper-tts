...
async function multiVoiceConversation(ctx: WOPRPluginContext) {
  ...
  const conversation = [
    { speaker: "WOPR", voice: "en_US-joe-medium", text: "Shall we play a game?" },
    { speaker: "User", voice: "en_US-amy-medium", text: "How about Global Thermonuclear War?" },
    { speaker: "WOPR", voice: "en_US-joe-medium", text: "Wouldn't you prefer a good game of chess?" },
  ];
  ...
  for (const line of conversation) {
    ...
    ctx.log.info(`${line.speaker}: ${line.text} (${result.audio.length} bytes)`);
